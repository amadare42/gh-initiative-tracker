...
export function ConnectionControl() {
    ...
    return <div className={ 'ConnectionControl-wrapper' } onClick={() => !isDrawerOpened ? setIsDrawerOpened(true) : null}>
        {
            isDrawerOpened ? <>
                <p onClick={closeDrawer}><ConnectionIcon status={status} /> { status == 'Room not found' ? status + ` (${lastRoomId})` : status }</p>
                {
                    roomId ? <>
                            <p>Room ID: <CopyToClipboard text={roomId} /> </p>
                ...
                <form onSubmit={joinRoom}>
                    ...
                    <button type={'submit'} disabled={isWaitingForServer || !isValidRoomId(editRoomId)}>Join</button>
                </form>
